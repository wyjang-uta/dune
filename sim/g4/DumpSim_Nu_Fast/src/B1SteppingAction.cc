...
void B1SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  for( size_t lp=0; lp < (*secondary).size(); lp++ )
  {
    ...
    if( abs(pdgEncoding) == 12 || abs(pdgEncoding) == 14 )
    {
      ...
      analysisManager->FillNtupleDColumn( 2, (G4double)(*secondary)[lp]->GetPosition().getX()/CLHEP::mm);
      analysisManager->FillNtupleDColumn( 3, (G4double)(*secondary)[lp]->GetPosition().getY()/CLHEP::mm);
      analysisManager->FillNtupleDColumn( 4, (G4double)(*secondary)[lp]->GetPosition().getZ()/CLHEP::mm);
